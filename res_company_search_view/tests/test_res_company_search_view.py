...
class TestResCompanySearchView(HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_edit_company(self):
        """
        Makes sure that all other fields from the company are editable, as usual.
        """
        company = self.company_alpha
        self.assertTrue(company)

        self.start_tour(
            f"odoo/companies/{company.id}",
            "res_company_search_view.tour_company_edit_address",
            login="admin",
        )

        self.assertEqual(company.partner_id.street, "ASDF")
        self.assertEqual(company.street, "ASDF")

    def test_search_groupby_country_company(self):
        """
        Makes sure the user can search and group by the countries of the companies.
        """
        self.start_tour(
            "/odoo/companies",
            "res_company_search_view.tour_company_search_groupby_country",
            login="admin",
        )
